...
const CollisionInformation CircleCollider::isColliding(const RectCollider& b) const {
	...
	if (pointLength(difference) < radius_m) {
		...
		// Kada se loptica odbije od splava, želimo joj dati dodatno ubrzanje u odreðenom smjeru.
		// Taj smjer ovisi o toèki na splavu u kojoj se dogodio udarac.
