	...
	bricksLayout_m{ bricksLayout }
{
	...
	int brickAreaWidth = windowW - position_m.x;
	float brickAreaHeight = windowH * 0.8f;

	SDL_Point playerPos;
	playerPos.x = position_m.x + static_cast<int>(brickAreaWidth * 0.45f);
	...
	float brickWidth = (brickAreaWidth - (columnCount_m - 1)*(columnSpacing_m / 20.0f)) / columnCount_m;
	float brickHeight = (brickAreaHeight - (rowCount_m - 1)*(rowSpacing_m / 20.0f)) / rowCount_m;
	...
	for (int i = 0; i < rowCount_m; i++)
		for (int j = 0; j < columnCount_m; j++) {
			...
			SDL_Point brickPos;
			brickPos.x = static_cast<int>(position_m.x + j * brickWidth + j * spacing.x);
			brickPos.y = static_cast<int>(position_m.y + i * brickHeight + i * spacing.y);
	...
	guiStatusBannerRect.y = static_cast<int>(windowH * 0.5f);
	guiStatusBannerRect.w = static_cast<int>(brickAreaWidth);
