...
bool AssetManager::loadTexture(std::string filePath, std::string id) {
	...
	if (newTexture == nullptr) {
		...
		SDL_Rect textureDim{0, 0, 0, 0};
		SDL_QueryTexture(newTexture, NULL, NULL, &textureDim.w, &textureDim.h);
		std::cout << "[AssetManager] Succeded loadTexture() on '" << filePath << "' !\n";
		std::cout << "[AssetManager] textureDim.w/h = " << textureDim.w << "/" << textureDim.h << "!\n";
...
TTF_Font* AssetManager::getFont() { return font_m; }
